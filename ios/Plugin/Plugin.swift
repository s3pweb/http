...
public class CAPHttpPlugin: CAPPlugin {
    ...
    @objc public func uploadFile(_ call: CAPPluginCall) {
        ...
        if(filePath != nil)
        {
            ...
            do {
                fullFormData = try generateFullMultipartRequestBodyFromBase64(base64, "image/jpg", name,  boundary)
            } catch let e {
                return call.reject("Unable to read file to upload", "UPLOAD", e)
            }
        }
    ...
    func generateFullMultipartRequestBodyFromBase64(_ base64: String, _ mimeType: String, _ name: String, _ boundary: String) throws -> Data {
        
        CAPLog.print("generateFullMultipartRequestBodyFromBase64")
